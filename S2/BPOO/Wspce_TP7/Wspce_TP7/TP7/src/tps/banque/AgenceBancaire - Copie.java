...
public class AgenceBancaire 
{
	...
	public void addCompte(Compte pfCompteAAjouter) throws ABCompteNullException, ABCompteDejaExistantException 
	{
		...
		Compte[] temp;
		temp = new Compte[this.tabCptes.length + 1];
		for (int i = 0; i < this.tabCptes.length; i++)
			temp[i] = this.tabCptes[i];
		this.tabCptes = temp;
		this.tabCptes[this.tabCptes.length-1] = pfCompteAAjouter;
	}
	...
	public void removeCompte(String pfNumCompte) throws ABCompteInexistantException
	{
		...
		for (int i = 0; i < rang; i++)
			temp[i] = this.tabCptes[i];
		for (int i = rang + 1; i < this.tabCptes.length; i++)
			temp[i-1] = this.tabCptes[i];
		this.tabCptes = temp;
	}
