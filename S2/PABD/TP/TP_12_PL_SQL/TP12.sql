...
--1.1.b
CREATE OR REPLACE PROCEDURE ajouterPays(vNom Pays.Nom%TYPE, vCap Pays.Capitale%TYPE, vSup Pays.Superficie%TYPE, vPop Pays.Population%TYPE, vIs3 Pays.Iso3%TYPE, vIs2 Pays.Iso2%TYPE, vCon Pays.NomC%TYPE) IS
	vTestP NUMBER;
	vTestI3 NUMBER;
	vTestI2 NUMBER;
	vTestC NUMBER;
...
--1.1.c
SET SERVEROUTPUT ON;
CREATE OR REPLACE PROCEDURE ajouterMontagnePays(vNomM Montagne.nom%TYPE, vAlt Montagne.altitude%TYPE, vChaine Montagne.chaine%TYPE, vNomP Pays.nom%TYPE) IS
	vTestM NUMBER;
	vTestP NUMBER;
	altError EXCEPTION;
	nomMError EXCEPTION;
...
--1.1.d
CREATE OR REPLACE PROCEDURE supprimerPays(vNom Pays.nom%TYPE) IS
	vTestP NUMBER;
	nomPError EXCEPTION;
...
EXEC ajouterMontagne('&pNom', &pAlt, '&pCha');

--2.1
CREATE OR REPLACE PROCEDURE AjouterFrontiere(vNomP Frontiere.nomp%TYPE, vNomF Frontiere.nomf%TYPE, vLong Frontiere.longueur%TYPE) IS
	n NUMBER;
	nomPError EXCEPTION;
	nomFError EXCEPTION;
